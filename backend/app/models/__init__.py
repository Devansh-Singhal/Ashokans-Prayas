...
class User(Base):
    ...
    parent_phone_number: Mapped[str | None] = mapped_column(
        String(16), nullable=True)
    consent_status: Mapped[str] = mapped_column(
        String(24), default=ConsentStatus.ACTIVE.value)
...
class Ticket(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(32), default=TicketStatus.REPORTED.value)
    ...
    resolution_photo_url: Mapped[str |
                                 None] = mapped_column(Text, nullable=True)
    is_commercial_adjacent: Mapped[bool] = mapped_column(
...
class Verification(Base):
    ...
    auditor_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("users.id"), nullable=False)
